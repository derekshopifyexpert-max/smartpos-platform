...
const USDT_CONTRACTS: Record<string, { chainId: number; contractAddress: string; decimals: number; nativeCurrency: string; requiredConfirmations: number }> = {
  ETHEREUM: {
    chainId: 1,
...
export default class BlockchainService {
  ...
  */
  ...
  private getNetworkConfig(networkName?: string) {
    const normalized = (networkName ?? process.env.BLOCKCHAIN_NETWORK ?? "ETHEREUM").trim().toUpperCase();
    const cfg = USDT_CONTRACTS[normalized] ?? null;

    if (!cfg) {
      throw new Error(`Unsupported EVM settlement network: ${normalized}. Supported networks: ${Object.keys(USDT_CONTRACTS).join(", ")}.`);
    }

    const configuredChainId = Number(process.env.BLOCKCHAIN_CHAIN_ID || cfg.chainId);
    const rpcUrl = String(process.env.BLOCKCHAIN_RPC_URL || process.env.RPC_URL || "").trim();
    const contractAddress = String(process.env.BLOCKCHAIN_USDT_CONTRACT_ADDRESS || cfg.contractAddress).trim();
    const decimals = Number(process.env.BLOCKCHAIN_USDT_DECIMALS || cfg.decimals);

    if (!rpcUrl) {
      throw new Error("Blockchain settlement requires a configured RPC URL. Set BLOCKCHAIN_RPC_URL or RPC_URL.");
    }

    if (!contractAddress) {
      throw new Error("Blockchain settlement requires a configured USDT contract address. Set BLOCKCHAIN_USDT_CONTRACT_ADDRESS.");
    }

    if (!ethers.isAddress(contractAddress)) {
      throw new Error(`Configured USDT contract address is invalid for ${normalized}: ${contractAddress}`);
    }
    ...
    return {
      ...
      requiredConfirmations: Number(process.env.BLOCKCHAIN_CONFIRMATIONS_REQUIRED || cfg.requiredConfirmations),
    };
  }

  private validateRealTxHash(txHash?: string): string {
  ...
  private async getRpcContext(networkName?: string) {
    const config = this.getNetworkConfig(networkName);
    const provider = new ethers.JsonRpcProvider(config.rpcUrl);
    const network = await provider.getNetwork();
    const expectedChainId = BigInt(config.chainId);

    if (BigInt(network.chainId) !== expectedChainId) {
      throw new Error(`RPC network mismatch: expected chain ID ${config.chainId} for ${config.name}, got ${network.chainId}.`);
    }

    const privateKey = String(process.env.BROADCAST_PRIVATE_KEY || "").trim();
    if (!privateKey) {
      throw new Error("Real blockchain broadcast requires a configured server-side BROADCAST_PRIVATE_KEY.");
    }
  ...
  private async findNetworkRecord(networkName: string) {
  ...
  */
  ...
  async createTransaction(data: {
    ...
    const realTxHash = this.validateRealTxHash(providedTxHash);
  ...
  async sendUsdtTransfer(data: {
    ...
    const network = String(data.network ?? "ETHEREUM").trim().toUpperCase();
    const asset = String(data.asset ?? "USDT").trim().toUpperCase();

    if (asset !== "USDT") {
      throw new Error(`Unsupported asset for on-chain settlement: ${asset}. Only USDT is currently supported.`);
    }

    const { config, provider, signer } = await this.getRpcContext(network);
    const destination = data.destinationAddress.trim();
    if (!ethers.isAddress(destination)) {
      throw new Error(`Destination wallet is not a valid EVM address: ${destination}`);
    }
    ...
    const decimals = await (async () => {
      ...
      const [tokenDecimals, tokenSymbol] = await Promise.all([
        tokenContract.decimals().catch(() => config.decimals),
        tokenContract.symbol().catch(() => "USDT"),
      ]);

      if (String(tokenSymbol).toUpperCase() !== "USDT") {
        throw new Error(`Configured USDT contract does not match the expected token for ${network}. Found symbol: ${String(tokenSymbol)}.`);
      }

      return { decimals: Number(tokenDecimals), symbol: String(tokenSymbol) };
    })();

    const amountUnits = ethers.parseUnits(String(data.amount), decimals.decimals);
    ...
    if (senderBalance < amountUnits) {
      throw new Error(`Insufficient USDT liquidity for this settlement. Available: ${ethers.formatUnits(senderBalance, decimals.decimals)} USDT.`);
    }

    const nativeBalance = await provider.getBalance(senderAddress);
    if (nativeBalance <= 0n) {
      throw new Error(`Insufficient ${config.nativeCurrency} balance to pay gas for the settlement.`);
    }

    const gasEstimate = await tokenContract.transfer.estimateGas(destination, amountUnits).catch((error: unknown) => {
      throw new Error(`Failed to estimate USDT gas for transfer: ${error instanceof Error ? error.message : String(error)}`);
    });
    ...
    const tx = await tokenContract.transfer(destination, amountUnits, txOverrides);
    const receipt = await tx.wait(1);

    if (!receipt) {
      throw new Error("Transaction broadcast succeeded but no blockchain receipt was returned.");
    }

    if (receipt.status === 0) {
      throw new Error(`On-chain USDT transfer reverted. Transaction hash: ${tx.hash}`);
    }

    const blockchainNetwork = await this.findNetworkRecord(network);
    if (!blockchainNetwork) {
      throw new Error(`Blockchain network ${network} is not configured in SmartPOS.`);
    }

    const createdTx = await this.createTransaction({
      blockchain: blockchainNetwork.id,
      walletId: data.walletId,
      fromAddress: senderAddress,
      toAddress: destination,
      amount: new Prisma.Decimal(String(ethers.formatUnits(amountUnits, decimals.decimals))),
      currency: asset as any,
      fee: receipt.gasUsed && receipt.effectiveGasPrice ? new Prisma.Decimal(String(ethers.formatUnits((receipt.gasUsed * receipt.effectiveGasPrice), 18))) : new Prisma.Decimal(0),
      gasPrice: receipt.effectiveGasPrice ? new Prisma.Decimal(String(receipt.effectiveGasPrice.toString())) : undefined,
      metadata: {
        ...((data.metadata ?? {}) as Record<string, unknown>),
        senderAddress,
        contractAddress,
        tokenSymbol: decimals.symbol,
      ...
      txHash: tx.hash,
      payload: { transactionHash: tx.hash, network, contractAddress, reference: data.reference ?? null } as Prisma.JsonValue,
    });

    return {
      txHash: tx.hash,
      blockchainTransactionId: createdTx.id,
      confirmations: createdTx.confirmations,
      requiredConfirmations: config.requiredConfirmations,
      result: {
        success: true,
        status: "BROADCASTED",
        message: "USDT transfer broadcast to blockchain",
        transactionHash: tx.hash,
        blockExplorerUrl: `${blockchainNetwork.explorerUrl ?? ""}/tx/${tx.hash}`.replace(/\/$/, ""),
        raw: receipt,
      },
    };
  }
