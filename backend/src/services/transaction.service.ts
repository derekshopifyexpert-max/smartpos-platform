...
export default class TransactionService {
  ...
  */
  ...
  async executePayment(data: {
    ...
    const providers = await this.gatewayService.activeProviders();
    ...
    const providerClient = this.providerManager.getProvider(provider.name);
    const endpoint = provider.baseUrl ?? "/payments";
    const method = "POST";
    const requestBody = {
    ...
    const gatewayRequest = await this.gatewayService.createGatewayRequest({
    ...
    const providerResponse = await providerClient.createPayment({
    ...
    await this.app.prisma.transaction.update({
      ...
      data: {
        gatewayTransactionId:
          providerResponse.transactionId ?? providerResponse.reference ?? null,
    ...
    const conversion = await this.exchangeService.createConversion({
      ...
      exchangeProvider: provider.name,
    });

    await this.gatewayService.createGatewayResponse({
      gatewayRequestId: gatewayRequest.id,
      statusCode: 200,
      responseBody: JSON.parse(JSON.stringify(providerResponse)),
      responseHeaders: {},
      responseTime: 150,
    });
    ...
    return {
      ...
      gateway: {
        ...
        transactionId: providerResponse.transactionId ?? null,
  ...
  */
  ...
  async settleTransaction(data: {
    ...
    */
    ...
    await this.transitionTransactionStatus({
      transactionId: data.transactionId,
      from: TransactionStatus.CAPTURED,
