...
return (
request.user as ExchangeUser | undefined
)?.merchantId;
}
...
*
* Note:
* The current Quidax Ramp integration requires a supported
* fiat currency and a network for BUY quotes.
  */
  getRealQuote = async (
  request: FastifyRequest,
  reply: FastifyReply
  ...
  * The current provider adapter requires a network for
  * Quidax Ramp BUY quotes.
  *
  * Do not silently invent a network.
    */
    if (
    side === "BUY" &&
    !network
  ...
  * getRealQuote currently accepts the provider interface
  * request shape. The network is passed through as an
  * additional property so the Quidax adapter can use it.
    */
    const quote =
